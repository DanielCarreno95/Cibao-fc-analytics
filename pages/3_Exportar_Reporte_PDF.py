...
def main():
    ...
    if not data_available or load_per90_func is None or load_cibao_func is None:
        st.error("⚠️ Error: No se pudieron importar las funciones de carga de datos. Por favor, verifica que los módulos estén disponibles.")
        return
    ...
    # STEP 1: SELECT TEAM
    # ===========================================
    st.markdown("### 1️⃣ Seleccionar Equipo Rival")
    ...
    # ===========================================
    st.markdown("### 2️⃣ Seleccionar Elementos para el Reporte")
    ...
    with st.expander("📊 **Gráficos de Radar (Comparación)**", expanded=True):
        ...
        if include_radar_charts:
            ...
            if not selected_radar_types:
                st.warning("⚠️ Por favor, selecciona al menos un tipo de gráfico de radar.")
            else:
                ...
                if not selected_radar_metrics:
                    st.warning("⚠️ Por favor, selecciona al menos una métrica para los gráficos de radar.")
                else:
    ...
    # ===========================================
    st.markdown("### 3️⃣ Generar y Descargar Reporte")
            ...
            try:
                ...
                if include_recent_form:
                    try:
                        ...
                        st.warning(f"⚠️ Error generando forma reciente: {e}")
                ...
                if include_key_metrics:
                    try:
                        ...
                        st.warning(f"⚠️ Error generando métricas clave: {e}")
                ...
                if include_radar_charts and selected_radar_types and selected_radar_metrics:
                    try:
                        ...
                        st.warning(f"⚠️ Error generando gráficos de radar: {e}")
                        import traceback
                        st.code(traceback.format_exc())
