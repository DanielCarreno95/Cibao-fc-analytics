        ...
        try:
            ...
            if "TeamStats" in xls.sheet_names:
                ...
                if "Team" in df.columns and len(df) > 0 and df["Team"].iloc[0] == "TeamStats":
                    ...
                    if "Match" in df.columns:
                        ...
                        if all_rows:
                            ...
                            raise RuntimeError(f"⚠️ No se encontraron partidos de Cibao en el archivo Excel")
                    else:
                        raise RuntimeError(f"⚠️ No se pudo determinar los equipos desde el archivo Excel")
                else:
                    # Filter for Cibao only if Team column is valid
                    if "Team" in df.columns:
                        df = df[df["Team"].str.lower() == "cibao"].copy()
                        if df.empty:
                            raise RuntimeError(f"⚠️ No se encontraron datos de Cibao en el archivo Excel")
            else:
                ...
                if sheet_names:
                    ...
                    raise RuntimeError(f"⚠️ El archivo Excel no contiene hojas")
        except Exception as e2:
            raise RuntimeError(f"⚠️ Error al leer el Excel: {e2}")
    except Exception as e:
