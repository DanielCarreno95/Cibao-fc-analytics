...
def extract_match_details(json_file: Path) -> Tuple[str, str, str]:
    ...
    try:
        ...
        print(f"    ⚠️  Unable to read JSON for naming: {err}")
...
def rename_export(json_file: Path, original_suffix: str, clean_suffix: str) -> Optional[Path]:
    ...
    if not source_file.exists():
        print(f"    ⚠️  Expected output not found: {source_file.name}")
        return None
        ...
        for json_file in json_files:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"    ⚠️  Player stats warning: {result.stderr[:120]}")
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"    ⚠️  Team stats warning: {result.stderr[:120]}")
            except Exception as exc:
